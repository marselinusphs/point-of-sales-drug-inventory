...
<!-- Kolom Kasir -->
<div class="bg-purple p-2 text-white" style="border-radius:0.25rem; background: #00105b; height:50px;">
  <h4 class="mb-0">KASIR</h4>
</div>

<!-- Kolom Tanggal-->
<div class="bg-purple p-2 text-white" style="border-radius:0.25rem; background:#00105b; height:50px; margin-top:10px;">
...
<form method="post">
  <div class="row">
    ...
    <div class="col-sm-4 col-md-4 col-lg-3 mb-3">
      <label class="small text-muted mb-1" style="margin-top:20px;">Kode Produk</label>
      <div class="position-relative">
        <input type="text" name="Ckdproduk" class="form-control form-control-sm" list="datalist1" autocomplete="off" onchange="changeValue(this.value)" required autofocus>
    ...
    <div class="col-sm-4 col-md-4 col-lg-3 mb-3" style="display:none">
      <label class="small text-muted mb-1" style="margin-top:20px;">Nama Produk</label>
    ...
    <div class="col-8 col-sm-4 col-md-4 col-lg-2 mb-3">
      <label class="small text-muted mb-1" style="margin-top:20px;">Harga</label>
    ...
    <div class="col-4 col-sm-4 col-md-4 col-lg-1 mb-3">
      <label class="small text-muted mb-1" style="margin-top:20px;">Qty</label>
...
if (isset($_POST['InputCart'])) {
  ...
  $InputQty = htmlspecialchars($_POST['Cqty']);
  ...
  $tgl = date("Y-m-d");
  $cekkodebarang = mysqli_query($conn, "SELECT produk.*, min(expired.tgl) as exp FROM produk inner join expired on produk.kode_produk = expired.kode_produk GROUP BY idproduk having exp>'$tgl' AND kode_produk='$Input1';");

  $rows = [];
  while ($row = mysqli_fetch_assoc($cekkodebarang)) :
    $rows[] = $row;
  endwhile;
  // var_dump($rows);
  // var_dump($rows[0]['stok']);
  if (mysqli_num_rows($cekkodebarang) == 0) {
    // TAMPILKAN WARNING JIKA PEGAWAI TIDAK MASUKIN KODE BARANG YG BENAR
    echo "Warning: Kode Produk tidak tersedia Atau Produk Sudah Kadaluarsa";
  } elseif ($InputQty < 1) {
    // TAMPILKAN WARNING JIKA QTY 0 atau mines
    echo "Warning: Qty tidak boleh bernilai mines";
  } elseif ($InputQty > $rows[0]['stok']) {
    // TAMPILKAN WARNING JIKA QTY 0 atau mines
    echo "Warning: Qty melebihi stok yang tersedia";
  } else {
    ...
    if ($liat > 0) {
      ...
      if ($liatlg > 0) {
        ...
        $cekkodebarang = mysqli_query($conn, "SELECT * FROM produk WHERE kode_produk = '$Input1'");
...
?><script>
       ...
       values('$inv_c','$Input1','$Input2','$Input3','$hrg_m','$ii',$Input3*$ii)");
                  if ($tambahdata) {
                    echo '<script>window.location="index.php"</script>';
                  } else {
                    echo '<script>window.location="index.php"</script>';
      ...
      values('$oi','$Input1','$Input2','$Input3','$hrg_m','$ii',$Input3*$ii)");
                  if ($tambahuser) {
                    echo '<script>window.location="index.php"</script>';
                  } else {
                    echo '<script>window.location="index.php"</script>';
          ...
          $noinv = $DataInv['invoice'];
                  ?>
<div class="bg-purple p-2 text-white" style="border-radius:0.25rem; background:#3DABCE;">
  ...
  <tbody>
    ...
    ?>
      <tr>
        ...
        <td><a class="btn btn-danger btn-xs" data-toggle="modal" data-target="#Hapus<?php echo $d['idcart']; ?>" style="color:white;">
            <i class="fas fa-trash-alt fa-xs mr-1" style="color:white;"></i>Hapus</a>
          <!-- Modal Hapus -->
          <div class="modal fade" id="Hapus<?php echo $d['idcart']; ?>" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
              <div class="modal-content border-0">
                <div class="modal-body text-center">
                  ...
                  <a href="?hapus=<?php echo $d['idcart']; ?>" class="btn btn-primary px-4">Iya</a>
                </div>
              </div>
            </div>
            <!-- end Modal Hapus -->
...
<!-- Kolom Total Bayar -->
<div class="bg-light p-3" style="border-radius:0.25rem; margin-top:10px;">
  <div class="col-md-16 col-lg-6 mb-2">
    <div class="col-md-4 col-lg-6 mb-2 text-primary text-right">
      ...
      <h2 style="font-weight:600; text-align:right;">Rp. <?php echo ribuan($itungtrans3) ?></h2>
    </div>
  </div>
</div>
    ...
    <form method="post">
      ...
      <div class="row">
        ...
        <div class="col-8 col-sm-8 col-md-8 col-lg-9 mb-2">
          <input type="text" name="pembayaran" onchange="procesBayar()" class="form-control form-control-sm" id="pembayaran" placeholder="0" required autocomplete="off" style="margin-top:10px;">
        </div>
...
if (isset($_POST['import'])) {
  ...
  while ($d = mysqli_fetch_array($data_cart)) {
    // var_dump($d);
    $banyaknyaa = $d['qty'];
    $kode_produk_ygdiubah = $d['kode_produk'];
    // echo $banyaknyaa, $kode_produk_ygdiubah;
    mysqli_query($conn, "UPDATE produk SET stok=stok-'$banyaknyaa' WHERE kode_produk='$kode_produk_ygdiubah'");
  }

  if ($Ikembalian < 0) {
    // TAMPILKAN WARNING JIKA INPUT PEMBAYARAN KURANG DARI TOTAL HARGA
